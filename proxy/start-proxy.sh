...
if [ "${AI_HTTPS_PORT:-443}" = "443" ]; then
  AI_HTTPS_ORIGIN="https://${LOCALHOST_AI_HOSTNAME:-${AI_HOSTNAME:-ai.localhost}}"
else
  AI_HTTPS_ORIGIN="https://${LOCALHOST_AI_HOSTNAME:-${AI_HOSTNAME:-ai.localhost}}:${AI_HTTPS_PORT}"
fi
